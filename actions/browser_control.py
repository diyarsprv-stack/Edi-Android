...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".edi_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".edi_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using EDI profile")
                edi = str(Path.home() / ".edi_profiles" / "firefox_edi")
                Path(edi).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(edi, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".edi_profiles" / "safari")
        ...
        edi_profile = str(Path.home() / ".edi_profiles" / self.browser_name)
        Path(edi_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with EDI profile: {edi_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(edi_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[Browser] ✅ Launched [{label}] with EDI profile")
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "edi_screenshot.png")
